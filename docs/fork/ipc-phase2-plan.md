...
pub struct IpcPromptGuardConfig {
    ...
    /// At 0.65: allows command_injection through (0.6 > 0.65 ✗).
...
- Unit: command_injection pattern (score 0.6) blocked at sensitivity 0.55 (0.6 > 0.55 ✓)
- Unit: command_injection pattern NOT blocked at sensitivity 0.65 (0.6 > 0.65 ✗)
...
let ipc_leak_detector = if ipc_enabled {
    // LeakDetector sensitivity aligns with PromptGuard: 0.7 default catches
    // generic secrets (password=, token=) in addition to API keys.
...
The `secret:*` namespace (L0-L1 only) should bypass leak detection — it's meant for secrets:
...
if from_level == to_level && from_level >= 2 {
    if let Some(ref sid) = body.session_id {
        ...
        if count >= max as i64 {
            // Send escalation to the configured coordinator
            let escalation_payload = serde_json::json!({
                "type": "session_limit_exceeded",
                "session_id": sid,
            ...
            let _ = db.insert_message(
                ...
                &coordinator,       // to: configured coordinator (not arbitrary L1)
